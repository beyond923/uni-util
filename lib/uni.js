...
export function toast(title, options = {}) {
  uni.showToast(typeof title === 'object' ? title : { icon: 'none', ...options, title })
}

;['success', 'loading'].forEach(icon => {
  toast[icon] = function (title, options = {}) {
    toast(typeof title === 'object' ? { ...title, icon } : { ...options, title, icon })
  }
})
...
export function loading(title = '加载中', mask = true, options = {}) {
  uni.showLoading({ ...options, title, mask })
}
...
export function modal(content = '', title = '提示', options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (typeof content === 'object') {
      ...
      _options = { ...title, content }
    } else {
      _options = { ...options, title, content }
    }
        ...
        bitmap.save(
          ...
          res => {
            ...
            uni.saveImageToPhotosAlbum({
              filePath: res.target,
              success: () => resolve(res.target),
              fail: reject,
              complete: () => bitmap.clear()
            })
          },
          reject
        )
...
/**
 * 生成图片二维码 base64
 * @param {text} text 生成的文字
 * @param {object} options 配置选项
 */
export function qrcode(text, options) {
  const image = qr.imageSync(text, options)
  return 'data:image/png;base64,' + uni.arrayBufferToBase64(image)
}
